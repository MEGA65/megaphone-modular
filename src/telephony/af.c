			...
			uint8_t fastP)
{
  ...
  case AF_SMS:
    ...
    read_record_by_id(0,USABLE_SECTORS_PER_DISK - 1,buffers.textbox.draft_record_marker);
  ...
  case AF_CONTACT_LASTNAME:
  case AF_CONTACT_PHONENUMBER:
    if (!fastP) try_or_fail(contact_read(contact_id,buffers.textbox.contact_record));
...
char af_store(char active_field, uint16_t contact_id)
{
  ...
  case AF_SMS:
    try_or_fail(mount_contact_qso(contact_id));
    // Write record + record marker
    write_record_by_id(0,USABLE_SECTORS_PER_DISK - 1,
		       buffers.textbox.draft_record_marker);
    break;
  case AF_CONTACT_FIRSTNAME:
  case AF_CONTACT_LASTNAME:
  case AF_CONTACT_PHONENUMBER:
